...
public class DateUtil {
    ...
    public static List<Integer> getDateAndTimeForString(String date) {
        ...
        try {
            if (!TextUtils.isEmpty(date)) {
                String[] dates = date.split("-|:| ");
                if (dates.length >= 5) {
                    if (null != dates[0]) list.add(Integer.parseInt(dates[0]));
                    if (null != dates[1]) list.add(Integer.parseInt(dates[1]));
                    
                    if (null != dates[2]) list.add(Integer.parseInt(dates[2]));
                    
                    if (null != dates[3]) list.add(Integer.parseInt(dates[3]));
                    
                    if (null != dates[4]) list.add(Integer.parseInt(dates[4]));
                    return list;
                } else {
                    return null;
                }
    ...
    //农历地支
    String[] mtwelve = {
            "null", "子（鼠）", "丑（牛）", "寅（虎）", "卯（兔）", "辰（龙）", "巳（蛇）", "午（马）", "未（羊）", "申（猴）", "酉（鸡）", "戌（狗）", "亥（猪）"
    };
