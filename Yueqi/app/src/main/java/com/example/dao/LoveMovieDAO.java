...
public class LoveMovieDAO {
    ...
    public List<ChuiGuanYueQi> queryAllFood() {
        String sql = "select * from tb_food";
        Cursor cursor = null;
        List<ChuiGuanYueQi> foodList = new ArrayList<ChuiGuanYueQi>();
        try {
            cursor = db.rawQuery(sql, null);
            while (cursor.moveToNext()) {
                ChuiGuanYueQi food = new ChuiGuanYueQi();
    ...
    // 增
    public void add(ChuiGuanYueQi food) {
        try {
